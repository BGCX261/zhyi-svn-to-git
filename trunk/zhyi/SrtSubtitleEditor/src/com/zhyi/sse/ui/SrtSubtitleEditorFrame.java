 ...
 */
public class SrtSubtitleEditorFrame extends JFrame
        implements ActionListener, KeyListener, ListSelectionListener, PropertyChangeListener {
    ...
    private void initMenuBar() {
        ...
        // File menu.
        openMenuItem = SwingToolkit.createMenuItem("Open...", KeyEvent.VK_O,
                KeyStroke.getKeyStroke(KeyEvent.VK_O, KeyEvent.CTRL_DOWN_MASK), this);
        saveMenuItem = SwingToolkit.createMenuItem("Save", KeyEvent.VK_S,
                KeyStroke.getKeyStroke(KeyEvent.VK_S, KeyEvent.CTRL_DOWN_MASK), this);
    ...
    private void initContentPane() {
        ...
        saveSubtitleButton = SwingToolkit.createButton(
                "<html><center>Save Subtitle<br>(Ctrl + Enter)</center></html>", null, this);

        JPanel panel = new JPanel();
        GroupLayout pgl = SwingToolkit.createGroupLayout(panel, false, true);
        pgl.setHorizontalGroup(pgl.createSequentialGroup()
            .addGroup(pgl.createParallelGroup()
                .addComponent(indexLabel).addComponent(beginTimeLabel).addComponent(endTimeLabel))
            .addGroup(pgl.createParallelGroup()
                .addComponent(indexTextField, 96, 96, 96)
                .addComponent(beginTimeTextField, 96, 96, 96)
                .addComponent(endTimeTextField, 96, 96, 96)));
        pgl.setVerticalGroup(pgl.createSequentialGroup()
            .addGroup(pgl.createParallelGroup(Alignment.BASELINE)
                .addComponent(indexLabel).addComponent(indexTextField))
            .addGroup(pgl.createParallelGroup(Alignment.BASELINE)
                .addComponent(beginTimeLabel).addComponent(beginTimeTextField))
            .addGroup(pgl.createParallelGroup(Alignment.BASELINE)
                .addComponent(endTimeLabel).addComponent(endTimeTextField)));

        GroupLayout gl = SwingToolkit.createGroupLayout(getContentPane(), true, true);
        gl.setHorizontalGroup(gl.createParallelGroup()
            .addComponent(srtTableScrollPane)
            .addGroup(gl.createSequentialGroup()
                .addComponent(panel).addComponent(contentScrollPane)
                .addComponent(saveSubtitleButton, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE)));
        gl.setVerticalGroup(gl.createSequentialGroup()
            .addComponent(srtTableScrollPane)
            .addGroup(gl.createParallelGroup()
                .addComponent(panel).addComponent(contentScrollPane).addComponent(saveSubtitleButton)));
    ...
    private void openSrtFile() {
        ...
        if ((srtFileChooser.showOpenDialog(this)) == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                List<Subtitle> subtitles = SrtToolkit.parseSrtFile(
                        srt, Context.getCharset());
    ...
    private void promptToSave() {
        ...
        int option = JOptionPane.showConfirmDialog(this,
                "File has been changed. Do you want to save it?", "Warning",
    ...
    private void saveSrtFile() {
        try {
            SrtToolkit.saveSrtFile(srtTableModel.getSubtitles(),
                    srtFile, Context.getCharset());
            updateSavingStatus(true);
        } catch (IOException ex) {
            SwingToolkit.showRootCause(ex, this);
        }
    ...
    private void addSubtitle(boolean addBefore) {
        ...
        // If no row is selected, add the subtitle before the first row if
        // "Add Before" was clicked, or after the last one if "Add After" was
        // clicked.
        if (index == -1) {
    ...
    private void removeSelectedSubtitles() {
        ...
        // If there's no selection, "Remove" menu item will be disabled, so here
        // there'return always some rows selected.
        int option = JOptionPane.showConfirmDialog(this,
                "Do you really want to remove the selected subtitle(s)?",
    ...
    private void moveUpSelectedSubtitles() {
        int[] rows = srtTable.getSelectedRows();
        // If there's no selection, "Move Up" menu item will be disabled.
        int first = rows[0];
    ...
    private void moveDownSelectedSubtitles() {
        int[] rows = srtTable.getSelectedRows();
        // If there's no selection, "Move down" menu item will be disabled.
        int first = rows[0];
        int last = rows[rows.length - 1];
    ...
    private void importSubtitles(final SubtitleImporter si) {
        ...
        new SwingWorker<List<Subtitle>, Void>() {
            ...
            protected List<Subtitle> doInBackground() throws Exception {
                return SrtToolkit.parseSrtFile(
                        si.getSourceFile(), Context.getCharset());
            }
            ...
            protected void done() {
                try {
                    ...
                    srtTable.setRowSelectionInterval(
                            index, index + subtitles.size() - 1);
                    updateSavingStatus(false);
                } catch (Exception ex) {
    ...
    private void exportSubtitles(SubtitleExporter se) {
        ...
        try {
            SrtToolkit.saveSrtFile(srtTableModel.getSubtitles().subList(beginRow, endRow),
                    se.getTargetFile(), Context.getCharset());
        } catch (IOException ex) {
            ex.printStackTrace();
        }
    }
    ...
    private void saveSubtitle() {
        // If no row is selected, editing components will be disabled.
        int index = srtTable.getSelectedRow();
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (src == openMenuItem) {
            ...
            JOptionPane.showMessageDialog(this,
                    ...
                    + "Copyright &copy; 2009-2010 Zhao Yi (shinzey@msn.com)"
                    + "<br>Licensed under GNU General Public License Version 3</html>",
                    "About SRT Subtitle Editor", JOptionPane.INFORMATION_MESSAGE);
